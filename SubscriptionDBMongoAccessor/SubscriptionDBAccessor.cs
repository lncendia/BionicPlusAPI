...
public class SubscriptionDBAccessor
{
    ...
    public async Task<string> CreateSubscription(CreateSubscriptionModel model, decimal sale = 0)
    {
        var filter = Builders<MongoPlan>.Filter.Where(p => p.Id == model.PlanId);
        var plan = (await _plansCollection.FindAsync(filter)).FirstOrDefault();
    
        if (plan == null)
        {
            throw new NotFoundException($"plan not found id = {model.PlanId}");
        }
        ...
        MongoSubscription subscription;
        
        // Handle subscription from Robokassa 
        if (model.FirstSubscriptionId == null)
        {
            subscription = new MongoSubscription
            {
                ...
                InvoiceId = ParseInvoiceId(model.InvoiceId),
                TransactionId = null,
                isRecurrent = true,
                Limits = plan.Limits,
                PlanId = plan.Id,
        ...
        // Handle subscription from another payment system
        else
        {
            var firstSubscriptionFilter = Builders<MongoSubscription>.Filter.Where(s => s.Id == model.FirstSubscriptionId);
            var firstSubscription = (await _subscriptionsCollection.FindAsync(firstSubscriptionFilter)).FirstOrDefault();
            ...
            subscription = new MongoSubscription
            {
                ...
                InvoiceId = firstSubscription.InvoiceId,
                TransactionId = model.TransactionId,
                isRecurrent = firstSubscription.isRecurrent,
                Limits = firstSubscription.Limits,
                PlanId = firstSubscription.PlanId,
                Currency = firstSubscription.Currency,
                Promocode = firstSubscription.Promocode,
                Discount = firstSubscription.Discount,
                Total = firstSubscription.Total
            };
        }
    ...
    private int ParseInvoiceId(string invoiceId)
    {
        if (string.IsNullOrEmpty(invoiceId)) return -1;
        
        if (int.TryParse(invoiceId, out var parsedInvoiceId))
        {
            return parsedInvoiceId;
        }

        throw new ArgumentException("Invalid invoiceId format.  Must be a valid integer.");
    }
}
